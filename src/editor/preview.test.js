...
test('Preview bundles the project, stores it, and opens a window at its URL', async () => {
    const it = editing();
    const opened = [];

    const result = await openPreview(it.workspace, { open: url => (opened.push(url), { closed: false }) });
...
test('what is stored is the project, not a handle to it', async () => {
    const it = editing();

    const result = await openPreview(it.workspace, { open: () => ({}) });
...
test('the live scene is saved first, so a preview plays what is on screen', async () => {
...
test('a blocked pop-up is not a failed preview, and says so', async () => {
    // The bundle IS stored and the link IS good; what is missing is permission.
    const it = editing();
    const said = [];

    const result = await openPreview(it.workspace, { open: () => null, report: message => said.push(message) });
...
test('the default opener does not read `noopener`\'s null as a blocked pop-up', async () => {
    ...
    try {
        const result = await openPreview(it.workspace, { report: message => said.push(message) });
...
test('a host with no window opener at all still tells the creator', async () => {
...
test('two previews are two windows on ONE game, sharing one identity', async () => {
    ...
    const first = await openPreview(it.workspace, { open: () => ({}) });
    const second = await openPreview(it.workspace, { open: () => ({}) });
...
test('no project is answered, not thrown', async () => {
    const said = [];

    assert.equal(await openPreview(null, { report: message => said.push(message) }), null);
    assert.equal(said.length, 1);
});
